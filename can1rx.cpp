...
void Can1Rx::canClose(){}

Can1MoveToThread::Can1MoveToThread(QObject* ){
    m_can.moveToThread(&m_thread);

    connect(&m_thread,&QThread::started,&m_can,&Can1Rx::canWork);
    connect(&m_can,&Can1Rx::dataRecved,this,&Can1MoveToThread::dataChangedSlot);
    connect(&m_thread,&QThread::finished,&m_can,&Can1Rx::canClose);
    m_thread.start();
}
...
void Can1MoveToThread::dataChangedSlot(struct can_frame msg)
{
    id = msg.can_id;
    byte0 = msg.data[0];
    byte1 = msg.data[1];
